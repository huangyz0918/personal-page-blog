...
$$ clip(f(x), 1 - \varepsilon, 1 + \varepsilon) $$ 的意思是说:

$$
clip(f(x), 1 - \varepsilon, 1 + \varepsilon) =
\begin{cases}
f(x) & if 1 - \varepsilon < f(x) < 1 + \varepsilon \\
1 - \varepsilon & if f(x) < 1 - \varepsilon \\
1 + \varepsilon & if f(x) > 1 + \varepsilon
\end{cases}
$$

这样就可以动态地将 $$ f(x) $$ 的值限定在 $$ 1 - \varepsilon $$ 到 $$ 1 + \varepsilon $$ 之间，达到两个分布不会相差太多地效果。而取最小值是因为，当 $$ A^{\theta^k}(s_t, a_t) > 0 $$ 时，我们希望 objective function 越大越好，但是一旦大过了 $$ 1 + \varepsilon $$，这个式子就不再有 benefit 了，因为不满足两个分布差别不要太大的这个约束条件，同理当 $$ A^{\theta^k}(s_t, a_t) < 0 $$ 的时候也是一样。
