...
其中 $$ A^{\theta}(s_t, a_t) = \sum_{t'=t}^{T_n} r_{t'}^{n} $$ 代入
...
我们可以给 $$ \sum_{t'=t}^{T_n} r_{t'}^{n} $$ 加上一个影响力衰弱参数，因为时间拖得越长，越前面发生的事件对后来的影响就会越小：

$$ A^{\theta}(s_t, a_t) = \sum_{t'=t}^{T_n} r_{t'}^{n} \gamma^{t'-t}, (0 < \gamma < 1) $$
