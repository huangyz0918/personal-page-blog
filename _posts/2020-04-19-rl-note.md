...
$$
s_a \rightarrow \left [ network V^{\pi} \right ] \rightarrow V^{\pi}(s_a) \leftrightarrow G_a
$$
...
即，针对某一个特定的 s ，$$ \pi' $$ 所采用的 action 一定不比 $$ \pi $$ 采取的有更小的 reward，那么加入每一步都 follow $$ \pi' $$ 给的 action:
...
& = \mathbb{E} \left [ r_{t+1} + r_{t+2} + V^{\pi}(s_{t+2}) | ... \right ] \\
& \leq \mathbb{E} \left [ r_{t+1} + r_{t+2} + Q^{\pi}(s_{t+2}, \pi'(s_{t+2})) | ... \right ] ... = V^{\pi'}(s)
