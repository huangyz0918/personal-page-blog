...
$$
b \approx \mathbb{E}\left [ R(\tau) \right ] 
$$
...
$$ 
\nabla \overline{R_{\theta}} \approx \frac{1}{N} \sum_{n=1}^{N} \sum_{t=1}^{T_n} \sum_{t'=t}^{T_n} (r_{t'}^{n} \cdot r_{t'}^{n} - b) \nabla \log p_{\theta}(a_t^n | s_t^n) 
$$ 
...
上述梯度中分布 $$ p_{\theta}(\tau) $$ 就变了，之前在老的 $$ \tau \sim p_{\theta}(\tau) $$ 上面采样的数据就没用了，必须抛弃掉。

所以针对这种 On-Policy 研究人员希望能够在不影响 agent 与环境互动的前提下持续地对我们需要的 agent 进行更新。Off-Policy 应运而生，这里主要讲 PPO/TRPO 和 PPO2 这几种 Off-Policy 的方法。
...
将这种方式应用到 policy gradient 上面，我们可以得到：
...
假设 $$ \pi_{\theta'}(s_t) \approx \pi_{\theta}(s_t) $$，那么上面的式子可以写成：

$$
\mathbb{E}_{(s_t, a_t) \sim \pi_{\theta'}} \left [ \frac{p_{\theta}(a_t | s_t)}{p_{\theta'}(a_t | s_t)} A^{\theta}(s_t, a_t) \nabla \log p_{\theta}(\tau) \right ]
$$ 

在这里我们用 $$ \pi_{\theta'} $$ 去估计 $$ \pi_{\theta} $$ 的分布，实际上就是用 agent $$ \pi' $$ 去和环境互动，根据其互动的数据去更新我们的 policy。这里 Important Sampling 其实有一个问题，虽然两个分布的 mean 是一样的，但是他们的方差是不同的，在 sample 数量不够多的话，$$ \pi_{\theta'} $$ 的方差就会巨大，所以采样的时候我们应该尽可能的保持多的样本数据来保证准确率，同时要保证两个分布不能差别太大。

借助之前的公式 $$ \nabla f(x) = f(x) \nabla \log f(x) $$，这里我们可以用 gradient 去反推原来的 object function，得到函数 $$ J^{\theta'}(\theta) $$：

$$ 
J^{\theta'}(\theta) = \mathbb{E}_{(s_t, a_t) \sim \pi_{\theta'}} \left [ \frac{p_{\theta}(a_t | s_t)}{p_{\theta'}(a_t | s_t)} A^{\theta}(s_t, a_t) \right ] 
$$

这个式子非常的直观：我们用 $$ \theta' $$ 去做 demonstration 从而优化我们想要的参数 $$ \theta $$。但是由于这个 object function $$ J^{\theta'}(\theta) $$ 牵扯到 Important Sampling，为了保证 Important Sampling 的效果，我们要让两个分布尽可能的相似，所以 PPO 就应运而生了: 在做训练的时候就要多加一个 constrain: $$ \beta KL(\theta, theta') $$，这一项代表着两个分布之间的 KL 距离，通过减去这一项，我们可以得到：

$$
J_{PPO}^{\theta'}(\theta) = J^{\theta'}(\theta) - \beta KL(\theta, theta')
$$

其中如果 $$ \beta KL(\theta, theta') $$ 越大 (即 $$ \theta $$ 和 $$ \theta' $$ 越不相似)，最终的 J_{PPO}^{\theta'}(\theta) 就会越小。通过优化这个式子，求其最大值，我们可以达到更好的 Reinforcement Learning 效果。
